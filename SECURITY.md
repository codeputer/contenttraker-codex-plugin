...
Since version 0.4.0, embedded `fast-uri` is pinned to patched version 3.1.4 or later. The MCP SDK still declares `@hono/node-server` in its development dependency tree, but this local stdio plugin does not import or bundle Hono's HTTP/static-server modules. Release validation rejects the artifact if that boundary changes.
...
Persistent entries are versioned envelopes bound to the ContentTraker environment, OAuth authority, audience/resource, public client, normalized required-user email policy, token issuer plus durable server subject, and configured credential profile. Issuer + subject is the account key; email remains optional host policy metadata. A profile cannot be overwritten by a different issuer or subject. The v2 namespace never loads or enumerates v0.1.2 session-derived entries.
...
Business API tools never launch a browser. They restore the identity-bound keyring entry or return `authentication_required`; interactive login begins only through the explicit login tools. The packaged plugin also fails before reading a stored credential unless Codex forwards both a named credential profile and exact required-user email. It fails closed when the keyring is missing, unavailable, corrupt, or locked.
...
Every business request verifies `/me` and live-authorizes exactly one canonical `ContentKeeperId` against the issuer + subject account's `/workspaces` response before routing. Writes repeat the ContentKeeper check immediately before mutation. `workspaceId` is accepted only as an equal-value compatibility alias. An explicit selector wins; otherwise a human-confirmed worktree marker or exact repository-root registry mapping is live-verified. Environment defaults and project-name-only mappings never authorize operations. Invalid, copied, stale, unauthorized, or divergent contexts fail closed and never silently select another ContentKeeper.

`.contenttraker-codex/context.json` is a strict, versioned, environment-indexed envelope bound to the normalized repository and physical Git worktree. It permits equal ContentKeeper/workspace compatibility identifiers, optional project provenance, names, timestamps, confirmation state, and plugin provenance only. Secret-like or unknown fields, alias mismatches, symlinks, oversized files, unsupported schemas, and repository/worktree mismatches are rejected. The marker and its atomic temporary files are ignored through Git's private `info/exclude`; the tracked `.gitignore` is not changed.
