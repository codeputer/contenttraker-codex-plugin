...
| WSL-native delegated OAuth | Live staging metadata succeeds inside that WSL distro, but device authorization is not advertised and no Linux browser exists; manual URL remains the selected interaction | Blocked at interaction/server capability (#11, #12) |
