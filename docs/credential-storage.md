...
One profile represents one durable ContentTraker subject. Use distinct profile names for distinct identities. A different subject cannot overwrite an existing profile; email remains a separate `GET /me` host-policy check.
...
Call `forget_contenttraker_credential` with confirmation `FORGET_CONTENTTRAKER_CREDENTIAL` to delete the selected local profile and clear in-process authorization state. This operation does not call a server revocation endpoint. Complete server-side revocation through the ContentTraker account security workflow.
